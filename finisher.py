...
def check_ssh_login(hostname, local_ssh_private_key):
    ...
    local_file_path = 'C:\\Users\\UKumar\\OneDrive - Genomadix\\Desktop\\Genomadix_Support_Client\\'
    remote_file_path = '/nfs/log/spyscript.log'
    sftp_client = client.open_sftp()

    # Change directory to /nfs/log
    sftp_client.chdir("/nfs/log")
    print('changed directory success')

    # Download the file
    remote_filename = remote_file_path.split('/')[-1]  # Extract filename from remote path
    local_file_path = local_file_path + '\\' + remote_filename  # Append filename to local directory
